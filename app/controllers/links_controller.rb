class LinksController < ApplicationController
	...
	def download
		puts "1 Ingreso a download attachment"
		@link.update_downloads
		@product = @link.product
		puts "3 cargo el product"
	...
	def download_attachment
		puts "ingreso a downlad attachment"
	...
	private
		...
		def find_attachment_link
			puts "ingreso a find attachment link"
			@link_attachment = LinkAttachment.find(params[:attachment_id]) # :attachment_id nombre que esta en routes.rb
		end


		def validate_attachment_link
			puts "ingreso a validate_attachment_link"
